...
public class SystemOrchestratorTest {
    ...
    public void allocate() throws InterruptedException{
        ...
        // Create threads
        Thread thread1 = new Thread(() -> {await(startLatch); assertTrue(systemOrchestrator.allocate(task1)); });
        Thread thread2 = new Thread(() -> {await(startLatch); assertTrue(systemOrchestrator.allocate(task2)); });
        Thread thread3 = new Thread(() -> {await(startLatch); assertTrue(systemOrchestrator.allocate(task3)); });
        ...
        // Wait for work to get started
        Thread.sleep(100);
        assertTrue(systemOrchestrator.hasPendingTasks());
